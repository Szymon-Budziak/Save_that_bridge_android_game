...
import unina.game.development.savethatbridge.liquidfun.ParticleFlag;
import unina.game.development.savethatbridge.liquidfun.ParticleGroup;
import unina.game.development.savethatbridge.liquidfun.ParticleGroupDef;
import unina.game.development.savethatbridge.liquidfun.ParticleGroupFlag;
import unina.game.development.savethatbridge.liquidfun.ParticleSystem;
...
public class MarblesGO extends GameObject {
    ...
    public void draw(Bitmap buffer, float _x, float _y, float _angle) {
        ...
        for (int i = 0; i < particleCount; i++) {
            int xint, yint;
            if (isLittleEndian) {
                xint = (particlePositions[i * 8 + bufferOffset] & 0xFF) | (particlePositions[i * 8 + bufferOffset + 1] & 0xFF) << 8 | (particlePositions[i * 8 + bufferOffset + 2] & 0xFF) << 16 | (particlePositions[i * 8 + bufferOffset + 3] & 0xFF) << 24;
                yint = (particlePositions[i * 8 + bufferOffset + 4] & 0xFF) | (particlePositions[i * 8 + bufferOffset + 5] & 0xFF) << 8 | (particlePositions[i * 8 + bufferOffset + 6] & 0xFF) << 16 | (particlePositions[i * 8 + bufferOffset + 7] & 0xFF) << 24;
            } else {
                xint = (particlePositions[i * 8] & 0xFF) << 24 | (particlePositions[i * 8 + 1] & 0xFF) << 16 | (particlePositions[i * 8 + 2] & 0xFF) << 8 | (particlePositions[i * 8 + 3] & 0xFF);
                yint = (particlePositions[i * 8 + 4] & 0xFF) << 24 | (particlePositions[i * 8 + 5] & 0xFF) << 16 | (particlePositions[i * 8 + 6] & 0xFF) << 8 | (particlePositions[i * 8 + 7] & 0xFF);
            }
